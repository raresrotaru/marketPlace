...
function loadProduct() {
  ...
  for (var i = 0; i < product.length; i++) {
    ...
    function getProduct(e) {
      fetch(`http://localhost:3000/api/socks/${e.target.parentNode.id}`, {
        headers: { Authorization: `Bearer ${sessionStorage.getItem("token")}` }
      })
        .then(r => r.json())
        .then(content => {
...
function addItemWishlist() {
  ...
  for (var i = 0; i < wishlistBtn.length; i++) {
    wishlistBtn[i].addEventListener('click', function (e) {
      ...
      fetch(`http://localhost:3000/api/socks/${e.target.previousSibling.previousSibling.id}`, {
        headers: { Authorization: `Bearer ${sessionStorage.getItem("token")}` }
      })
        .then(r => r.json())
        .then(content => {
...
function addItemCart() {
  ...
  for (var i = 0; i < cartBtn.length; i++) {
    cartBtn[i].addEventListener('click', function (e) {
      ...
      informationContainer.classList = "informationContainer"
      productContainer.classList = "productContainer"
      fetch(`http://localhost:3000/api/socks/${e.target.previousSibling.id}`, {
        headers: { Authorization: `Bearer ${sessionStorage.getItem("token")}` }
      })
        .then(r => r.json())
        .then(content => {
