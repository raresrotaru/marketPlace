...
function loadProduct() {
  ...
  for (var i = 0; i < product.length; i++) {
    ...
    function getProduct(e) {
      fetch(`http://localhost:3000/api/accessories/${e.target.parentNode.id}`, {
        headers: { Authorization: `Bearer ${sessionStorage.getItem("token")}` }
      })
        .then(r => r.json())
        .then(content => {
...
function addItemWishlist() {
  ...
  for (var i = 0; i < wishlistBtn.length; i++) {
    wishlistBtn[i].addEventListener('click', function (e) {
      ...
      fetch(`http://localhost:3000/api/accessories/${e.target.previousSibling.previousSibling.id}`, {
        headers: { Authorization: `Bearer ${sessionStorage.getItem("token")}` }
      })
        .then(r => r.json())
        .then(content => {
...
function addItemCart() {
  ...
  for (var i = 0; i < cartBtn.length; i++) {
    cartBtn[i].addEventListener('click', function (e) {
      ...
      informationContainer.classList = "informationContainer"
      productContainer.classList = "productContainer"
      fetch(`http://localhost:3000/api/accessories/${e.target.previousSibling.id}`, {
        headers: { Authorization: `Bearer ${sessionStorage.getItem("token")}` }
      })
        .then(r => r.json())
        .then(content => {
...
function searchProducts(e) {
  ...
  const textResult = mainContainer.appendChild(document.createElement("p"))
  textResult.innerHTML = `Your results for "${searchInput}" are:`
  fetch("http://localhost:3000/api/accessories", {
    headers: { Authorization: `Bearer ${sessionStorage.getItem("token")}` }
  })
    .then(r => r.json())
    .then(content => {
      ...
      for (var i = 0; i < content.length; i++) {
        if (content[i].info.material.toLowerCase().includes(searchInput)) {
          displayProducts();
          addItemWishlist();
          addItemCart();
        } else if (content[i].info.color.toLowerCase().includes(searchInput)) {
          displayProducts();
          addItemWishlist();
          addItemCart();
        } else if (content[i].info.brand.toLowerCase().includes(searchInput)) {
          displayProducts();
          addItemWishlist();
          addItemCart();
        } else if (content[i].info.form.toLowerCase().includes(searchInput)) {
          displayProducts();
          addItemWishlist();
          addItemCart();
        }
